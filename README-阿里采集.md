...
在LivePushActivity 中进行初始化，根据是否需要开启美颜来决定是否调用方法。

#### 2.创建

调用 `FURenderer` 类的  `onSurfaceCreated` 方法在 SDK 使用前加载必要的资源。

在LivePushActivity 类中，自定义Filter实现AlivcLivePushCustomFilter接口，customFilterCreate回调方法中执行FURenderer.onSurfaceCreated();
...
在LivePushActivity 类中，自定义Filter实现AlivcLivePushCustomFilter接口，customFilterProcess回调方法可以执行美颜操作
...
阿里云推流支持单输入： onDrawFrameSingleInput 

#### 4. 销毁

调用 `FURenderer` 类的  `onSurfaceDestroyed` 方法在 SDK 结束前释放占用的资源。

在LivePushActivity 类中AlivcLivePushCustomFilter接口中的customFilterDestroy回调方法进行销毁。

#### 5. 切换相机

调用 `FURenderer` 类 的  `onCameraChanged` 方法，用于重新为 SDK 设置参数。

在LivePushActivity 类中执行，缺少这个方法，会导致在切换摄像头时美颜失效。
